...
import { DollarOutlined, FileTextOutlined, UserOutlined, ClockCircleOutlined, SearchOutlined } from '@ant-design/icons';
...
const CustomsOfficerPayment: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPaymentHistory = async () => {
      try {
        const response = await fetch('https://localhost:7232/payment-history-show');
  ...
  useEffect(() => {
    const filteredData = payments.filter((payment) =>
      String(payment.paymentId).toLowerCase().includes(searchText.toLowerCase()) ||
      String(payment.userId).toLowerCase().includes(searchText.toLowerCase()) ||
      String(payment.declarationId).toLowerCase().includes(searchText.toLowerCase()) ||
      String(payment.productName).toLowerCase().includes(searchText.toLowerCase())
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'amount',
      key: 'amount',
      render: (amount: number) => <span><DollarOutlined /> {amount.toFixed(2)}</span>,
    },
    {
      title: 'Date',
      dataIndex: 'date',
      key: 'date',
      render: (date: string) => <span><ClockCircleOutlined /> {new Date(date).toLocaleDateString()}</span>,
    },
    {
      title: 'Status',
      dataIndex: 'status',
